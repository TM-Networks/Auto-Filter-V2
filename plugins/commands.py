...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    InlineKeyboardButton('Uᴘᴅᴀᴛᴇs', url='https://t.me/Movies_Workshop'),
                    InlineKeyboardButton('Sᴜᴘᴘᴏʀᴛ', url='https://t.me/+VOTnmaP0YCg0YzZl')
                  ]]
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            if BATCH_FILE_CAPTION:
                try:
                    f_caption=BATCH_FILE_CAPTION.format(file_name= '' if title is None else title, file_size='' if size is None else size, file_caption=title if f_caption is None else f_caption)
